...
export const useWorkspace = () => {
    ...
    const addPage = (title = 'Untitled Page', icon = '📝') => {
        const newPage = {
    ...
    const updatePageTitle = (pageId, title) => {
        ...
        setWorkspace((prev) => ({
            ...prev,
            pages: prev.pages.map((p) => p.id === pageId ? { ...p, title, updatedAt: new Date() } : p),
            updatedAt: new Date()
        }));
    ...
    const updatePageIcon = (pageId, icon) => {
        setWorkspace((prev) => ({
            ...prev, // Spread prev
            pages: prev.pages.map((p) => p.id === pageId ? { ...p, icon, updatedAt: new Date() } : p),
            updatedAt: new Date()
        }));
    };
    const updatePageCover = (pageId, cover) => {
        const oldPage = workspace.pages.find(p => p.id === pageId);
        if (!oldPage)
            return;
        setWorkspace((prev) => ({
            ...prev,
            pages: prev.pages.map((p) => p.id === pageId ? { ...p, cover: cover || undefined, updatedAt: new Date() } : p),
            updatedAt: new Date()
        }));
        // Create version history entry
    ...
    const addBlock = (pageId, type = 'paragraph') => {
        setWorkspace((prev) => ({
            ...prev, // Spread prev
    ...
    const updateBlock = (pageId, blockId, updates) => {
        setWorkspace((prev) => ({
            ...prev, // Spread prev
            pages: prev.pages.map((p) => p.id === pageId
                ? {
