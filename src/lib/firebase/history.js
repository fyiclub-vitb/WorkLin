import { collection, doc, getDocs, addDoc, query, orderBy, limit, serverTimestamp, getDoc } from 'firebase/firestore';
...
export const createVersion = async (pageId, oldPage, newPage, userId, userName = 'Unknown') => {
    // 1. Calculate Diff
    const diff = {};
...
export const restoreVersion = async (pageId, versionId, userId) => {
    ...
    const targetVersion = versionDoc.data();
...
export const reconstructPageAtVersion = async (pageId, versionId) => {
    // 1. Get Current Live Page
    const pageRef = doc(db, 'pages', pageId);
    const pageSnap = await getDoc(pageRef);
    if (!pageSnap.exists())
        throw new Error("Page does not exist");
    let currentPage = pageSnap.data();
    ...
    const querySnapshot = await getDocs(q);
    const allVersions = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
