 ...
 */
function formatBlockToHtml(block) {
 ...
 */
function formatTableToHtml(tableData) {
...
export async function exportPdf(page) {
    try {
        ...
        for (const block of page.blocks) {
            ...
            switch (block.type) {
                ...
                case "quote":
                    ...
                    pdf.line(pageMargin + 2, yPosition - 2, pageMargin + 2, yPosition + qHeight - 2);
                ...
                case "callout":
                    ...
                    pdf.setLineWidth(0.5);
                    pdf.line(pageMargin, yPosition - 2, pageMargin, yPosition + callHeight - 2);
                    pdf.setDrawColor(0, 0, 0);
                        ...
                        if (tableData && tableData.rows && tableData.rows.length > 0) {
                            ...
                            for (let i = 0; i < tableData.rows.length; i++) {
                                ...
                                for (const cell of row) {
                                    pdf.rect(xPos - 1, yPosition - 6, colWidth, 8, i === 0 ? "F" : "S");
                                    pdf.text(String(cell || "").substring(0, 10), xPos, yPosition - 2);
                                    xPos += colWidth;
                                }
                                yPosition += 8;
                                if (i === 0) {
        ...
        throw new Error("Failed to export as PDF. Please check the console for details.");
    }
}
/**
 ...
 */
export async function exportPdfAlternative(page) {
    try {
        // @ts-ignore - jsPDF doesn't have complete type declarations
        const { jsPDF } = await import("jspdf");
